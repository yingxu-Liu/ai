...
// Highlight changes the brightest part of the image
// The light param is a value to that increase or decrease the brightness of the image.
// If light is positive, the brightness will be increased.
// If light is negative, the brightness will be decreased.
func Highlight(img gocv.Mat, light int64) gocv.Mat {
	start := time.Now()
	defer func(start time.Time) {
		dur := time.Since(start)
		log.Printf("func highlight took %0.0fs to execute\n", dur.Seconds())
	}(start)
